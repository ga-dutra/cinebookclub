...
${(0, import_indent_string4.default)(
      relations.map(
        (f2) => `P extends '${f2.name}' ? ${this.wrapType(
          f2,
          `${getPayloadName(f2.outputType.type.name)}<S['${projection}'][P]${ifExtensions(
            ", ExtArgs",
            ""
          )}>`
        )} :`
...
${(0, import_indent_string9.default)(
      outputType.fields.filter((f2) => {
        ...
        const fieldTypeName = f2.outputType.type.name;
        return `${f2.name}?: boolean` + (f2.outputType.location === "outputObjectTypes" ? ` | ${getFieldArgName(f2, !this.dmmf.typeMap[fieldTypeName])}${ifExtensions("<ExtArgs>", "")}` : "");
      }).join("\n"),
      TAB_SIZE
    )}
} 
...
${(0, import_indent_string9.default)(
      outputType.fields.filter((field) => ifExtensions(field.outputType.location === "outputObjectTypes", true)).map((f2) => {
        const fieldTypeName = f2.outputType.type.name;
        return `${f2.name}?: boolean` + (f2.outputType.location === "outputObjectTypes" ? ` | ${getFieldArgName(f2, !this.dmmf.typeMap[fieldTypeName])}${ifExtensions("<ExtArgs>", "")}` : "");
      }).join("\n"),
      TAB_SIZE
...
${(0, import_indent_string9.default)(
        outputType.fields.filter((field) => field.outputType.location === "scalar").map((f2) => `${f2.name}?: boolean`).join("\n"),
        TAB_SIZE
      )}
}`;
    }, "")}
...
${(0, import_indent_string9.default)(
      fields.filter((f2) => f2.outputType.location === "outputObjectTypes" && f2.name !== "_count").map((f2) => {
        const fieldTypeName = f2.outputType.type.name;
        return `
${f2.name}<T extends ${getFieldArgName(f2, !this.dmmf.typeMap[fieldTypeName])}${ifExtensions(
          "<ExtArgs> = {}",
          "= {}"
        )}>(args?: Subset<T, ${getFieldArgName(f2, !this.dmmf.typeMap[fieldTypeName])}${ifExtensions(
          "<ExtArgs>",
          ""
        )}>): ${getReturnType({
          name: fieldTypeName,
...
function clientExtensionsQueryDefinition() {
  ...
  const allOperationsParam = /* @__PURE__ */ __name((modelNames2, indent15) => {
    const key = modelNames2.length > 1 ? `keyof Prisma.TypeMap<ExtArgs>` : `'${modelNames2[0]}'`;
    return `{
    ${indent15}$allOperations?: (args: Prisma.TypeMap<ExtArgs>[${key}][keyof Prisma.TypeMap<ExtArgs>[${key}]]['queryExtCbArgs']) => Prisma.TypeMap<ExtArgs>[${key}][keyof Prisma.TypeMap<ExtArgs>[${key}]]['result']
  ${indent15}}`;
  }, "allOperationsParam");
...
function clientExtensionsHookDefinition(name) {
  ...
  const modelNameUnion = lcModelNames.map((m2) => `'${m2}'`).join(" | ");
  return {
    ...
    ExtArgs extends runtime.Types.Extensions.Args = runtime.Types.Extensions.DefaultArgs,` : ""}
  >(extension: ((client: ${name === "defineExtension" ? "PrismaClient" : "this"}) => { $extends: { extArgs: Args } }) | {
    name?: string,
    result?: R & ${result.params}
    model?: M & ${model.params}
    query?: ${query.params}
    client?: C & ${client.params}
  })${name === "defineExtension" ? ":" : "=>"} ${name === "defineExtension" ? "(client: any) => PrismaClient<any, any, any, Args>" : `runtime.Types.Extensions.GetClient<PrismaClient<T, U, GlobalReject, runtime.Types.Extensions.MergeArgs<Args, ExtArgs, ${modelNameUnion}>>, Args['client'], ExtArgs['client']>`}${name === "defineExtension" ? "" : ")"};`,
...
get ${methodName}(): ${ifExtensions(
          `runtime.Types.Extensions.GetModel<Prisma.${m2.model}Delegate<GlobalReject, ExtArgs>, ExtArgs['model']['${lowerCase(m2.model)}']>`,
          `Prisma.${m2.model}Delegate<GlobalReject>`
        )};`;
      }).join("\n\n"),
      2
    )}
